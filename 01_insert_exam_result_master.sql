-- 마스터 정보 기준 반영 검사, 혈액검사 포함 패키지 고려함.
            ...
            select a.ptno               as ptno
              ...
              from (-- 기준치 적용 수치데이터
                    select /*+ INDEX(A 3E3C0E433E3C0E3E28_I02) INDEX(B 3E3243333E2E143C28_PK) INDEX(C 3C15332B3C20431528_PK) index(d 1543294D47144D3C0E3E283343_PK) index(E 3E3C3C5B0C233C3E28_PK) */
                         ...
                         , c.enfr_dt
                         , case
                                when decode(REGEXP_REPLACE(b.exrs_ncvl_vl,'^-?[0-9]+((\.[0-9]+)([Ee][+-][0-9]+)?)?', ''), null, to_number(b.exrs_ncvl_vl), '') between to_number(nvl(d.CLEANED_LWLM_VL,'0')) and to_number(nvl(d.CLEANED_UPLM_VL,'9999999')) then b.exrs_ncvl_vl
                           else ''
                           end
                           cleaned_vl
                         , b.exrs_ncvl_vl
                     ...
                     where a.ordr_ymd between to_date(&Y||'0101','yyyymmdd') and to_date(&Y||'0630','yyyymmdd')
